...
\section{Methodology}
This section presents the proposed framework for tourism review analytics. The methodology follows a structured pipeline consisting of three main phases: data preprocessing, feature engineering, and model validation. Table~\ref{tab:methodology_overview} provides a high-level summary of each phase.

\begin{table}[htbp]
\caption{Methodology framework overview}
\label{tab:methodology_overview}
\centering
\begin{tabular}{p{3.2cm}p{4.5cm}p{6cm}}
\toprule
\textbf{Phase} & \textbf{Key Tasks} & \textbf{Output} \\
\midrule
Dataset \& Preprocessing & Missing value removal, duplicate detection, location normalization (province/district extraction), user country inference & Cleaned, normalized dataset \\
\midrule
Feature Engineering & Location features (province, district, city, type), Temporal features (travel/pub month/year, review delay), Behavioral features (user contributions, helpful votes), Text features (title/review length) & Engineered feature set \\
\midrule
Modeling \& Validation & Train-test split, cross-validation, classification (Accuracy, Precision, Recall, F1-score) & Validated predictive models \\
\bottomrule
\end{tabular}
\end{table}
